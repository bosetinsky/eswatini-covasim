...
def make_randpop(sim, use_age_data=True, use_household_data=True, sex_ratio=0.55, microstructure=False):
...
def make_hybrid_contacts(pop_size, ages, contacts, school_ages=None, work_ages=None):
    ...
    layer_keys = ['h', 's', 'w', 'c']
    contacts = sc.mergedicts({'h':4.1, 's':33, 'w':20, 'c':20}, contacts) # Ensure essential keys are populated
