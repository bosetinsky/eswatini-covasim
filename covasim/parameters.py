...
def make_pars(set_prognoses=False, prog_by_age=True, **kwargs):
    ...
    pars['pop_size']     = 100000 # Number ultimately susceptible to CoV
    ...
    pars['start_day']  = '2020-03-15' # Start day of the simulation
    pars['end_day']    = '2020-12-31' # End day of the simulation
    pars['n_days']     = 60 # Number of days to run, if end_day isn't specified
    pars['rand_seed']  = 1 # Random seed, if None, don't reset
    pars['verbose']    = 1 # Whether or not to display information during the run -- options are 0 (silent), 1 (default), 2 (everything)

    # Rescaling parameters
    pars['pop_scale']         = 11   # Factor by which to scale the population -- e.g. pop_scale=10 with pop_size=100e3 means a population of 1 million
    pars['rescale']           = 1   # Enable dynamic rescaling of the population -- starts with pop_scale=1 and scales up dynamically as the epidemic grows
    pars['rescale_threshold'] = 0.05 # Fraction susceptible population that will trigger rescaling if rescaling
    pars['rescale_factor']    = 1.2  # Factor by which the population is rescaled
    ...
    pars['n_beds'] = 2.42  # The number of beds available for severely/critically ill patients (default is no constraint)
...
def reset_layer_pars(pars, layer_keys=None, force=False):
    ...
    d_iso_factor  = 0.2 # Assumed isolation factor (idm assumes 0.2)
    d_quar_factor = 0.3 # Assumed quarantine factor (idm assumes 0.3)
    ...
    if layer_keys is not None: # Create based on known population keys
        ...
        if pars['pop_type'] == 'random':
            ...
            if pars.get('contacts',    None) is None or force: pars['contacts']    = dict(h=4.1, s=33,  w=8,  c=20)   # Number of contacts per person per day, estimated
            if pars.get('dynam_layer', None) is None or force: pars['dynam_layer'] = dict(h=0,   s=0,   w=0,   c=1)    # Which layers are dynamic -- none by defaul
            ...
            if pars.get('iso_factor',  None) is None or force: pars['iso_factor']  = dict(h=0.3, s=0.0, w=0.0, c=0) # Multiply beta by this factor for people in isolation  (h=0.3, s=0.0, w=0.0, c=0.1)
            if pars.get('quar_factor', None) is None or force: pars['quar_factor'] = dict(h=0.8, s=0.0, w=0.0, c=0) # Multiply beta by this factor for people in quarantine (h=0.8, s=0.0, w=0.0, c=0.3)
    return
